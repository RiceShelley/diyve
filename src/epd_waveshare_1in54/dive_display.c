...
static void display_labeled_int(UWORD x, UWORD y, const char* label, size_t label_len, int32_t num, sFONT* font) {
...
void display_dive_entry(dive_log_entry_t* entry) {
    ...
    display_labeled_int(
        ...
        temp_y,
        temp_disp_str,
        sizeof(temp_disp_str),
        (int) c_to_f(entry->temperature_c),
        font
    );
    ...
    display_labeled_int(
        ...
        depth_y,
        depth_disp_str,
        sizeof(depth_disp_str),
        (int) m_to_ft(entry->depth_m),
        font
    );
