...
ms5803_inst_t *ms5803_init(precision p) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (ret < 0)
            panic("ms5803: I2C write failed reading coefficient %d\n", i);
        ret = i2c_read_blocking(MS5803_I2C_PORT, MS5803_I2C_ADDR, &coef_buff[0], 2, false);
        if (ret < 0)
            panic("ms5803: I2C read failed reading coefficient %d\n", i);
...
uint32_t getADCconversion(measurement m, precision p) {
    ...
    ret = i2c_write_blocking(MS5803_I2C_PORT, MS5803_I2C_ADDR, &txdata, 1, false);
    if (ret < 0)
        panic("ms5803: I2C write failed sending conversion command\n");
    ...
    ret = i2c_write_blocking(MS5803_I2C_PORT, MS5803_I2C_ADDR, &txdata, 1, false);
    if (ret < 0)
        panic("ms5803: I2C write failed sending ADC read command\n");

    // Read result
    ret = i2c_read_blocking(MS5803_I2C_PORT, MS5803_I2C_ADDR, rxdata, 3, false);
    if (ret < 0)
        panic("ms5803: I2C read failed reading ADC result\n");
